...
def main():
    ...
    upload_to_s3(census_data, "api", "census_acs5_2024.csv")
    ...
    d.s3_to_duckdb(
        ...
        "nppes_raw",
        BUCKET,
        f"{OBJECT_PATH}nppes/npidata_pfile_20050523-20250413.csv",
        logger,
    )
    ...
    d.s3_to_duckdb(
        ...
        "states_raw",
        BUCKET,
        f"{OBJECT_PATH}geographic/ssa_fips_state_county_2025.csv",
        logger,
    )
    ...
    d.s3_to_duckdb(
        ...
        "taxonomy_raw",
        BUCKET,
        f"{OBJECT_PATH}geographic/nucc_taxonomy_250.csv",
        logger,
    )

    ## Zip_county

    zip_county_raw = s3.get_raw_excel_data_from_AWS(
        BUCKET, f"{OBJECT_PATH}geographic/ZIP_COUNTY_032025.xlsx", logger
    )
